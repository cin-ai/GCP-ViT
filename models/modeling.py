...
# --- Weight key-value mapping constants ---
...
class DropPath(nn.Module):
    ...
    def forward(self, x):
        ...
        # handle tensors with different dimensions, not just 4D
...
class Block(nn.Module):
    def __init__(self, config, drop_path_ratio=0.0):
...
class Part_Selection_Module(nn.Module):
    """
    Select the most significant K patches based on attention maps from all layers.
    center_bias=True: Use Gaussian mask to force the model to focus on the center and ignore background.
    """
    def __init__(self, topk=4, center_bias=True):
        super(Part_Selection_Module, self).__init__()
        self.topk = topk
        self.center_bias = center_bias

    def get_gaussian_mask(self, num_patches, device):
        """Generate Gaussian mask with high center and low edges"""
        grid_size = int(math.sqrt(num_patches))
        if grid_size * grid_size != num_patches:
            return None # Cannot reshape to square grid
            
        x = torch.arange(grid_size, dtype=torch.float32, device=device)
        y = torch.arange(grid_size, dtype=torch.float32, device=device)
        yy, xx = torch.meshgrid(y, x) # Generate grid coordinates
        
        center = (grid_size - 1) / 2.0
        dist_sq = (xx - center)**2 + (yy - center)**2
        
        # The smaller sigma, the more focused; recommended grid_size / 2.5
        sigma = grid_size / 2.5 
        mask = torch.exp(-dist_sq / (2 * sigma**2))
        return mask.flatten()

    def forward(self, hidden_states, all_attn_weights):
        # hidden_states: [B, L, D]
        # all_attn_weights: list of [B, Heads, L, L]

        # 1. Aggregate attention (take CLS's attention to other patches)
        attns_list = []
        for attn in all_attn_weights:
            attns_list.append(attn[:, :, 0, 1:]) # [B, Heads, L-1]
        
        # Average across all layers and heads -> [B, L-1]
        avg_attn = torch.cat(attns_list, dim=1).mean(dim=1) 
        
        # 2. Apply Center Bias (if enabled)
        if self.center_bias:
            mask = self.get_gaussian_mask(avg_attn.shape[-1], avg_attn.device)
            if mask is not None:
                # Downweight edge regions
                avg_attn = avg_attn * mask.unsqueeze(0)
        
        # 3. Select Top-K
        _, indices = torch.topk(avg_attn, k=self.topk, dim=-1)
        
        # 4. Extract features
        B, K = indices.shape
        select_indices = indices + 1 # +1 because hidden_states[0] is CLS
        batch_indices = torch.arange(B, device=hidden_states.device).unsqueeze(1).expand(-1, K)
        
        selected_features = hidden_states[batch_indices, select_indices, :]
...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        # ==========================================
        # Core setting: Layer-wise DropPath Rate
        # ==========================================
        # If config doesn't have drop_path_rate, default to 0.1
        total_dp_rate = config.transformer.get('drop_path_rate', 0.1)
        # Generate linearly increasing drop rate: [0.0, ..., 0.1]
        dpr = [x.item() for x in torch.linspace(0, total_dp_rate, config.transformer["num_layers"])]
        
        for i in range(config.transformer["num_layers"] - 1):
            layer = Block(config, drop_path_ratio=dpr[i])
            self.layer.append(copy.deepcopy(layer))
        
        # Enable Center Bias to suppress background
        self.part_select = Part_Selection_Module(topk=4, center_bias=True)
        
        # Last Part Layer uses the maximum drop rate
        self.part_layer = Block(config, drop_path_ratio=dpr[-1])
    ...
    def forward(self, hidden_states):
        ...
        # 2. Top-K selection (with Center Bias)
        part_features = self.part_select(hidden_states, all_attn_weights)
        
        # 3. Concatenation
        concat_states = torch.cat([hidden_states, part_features], dim=1)
        
        # 4. Final fusion
        part_encoded, _ = self.part_layer(concat_states)
        part_encoded = self.part_norm(part_encoded)
